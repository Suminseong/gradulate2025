...
Chamkkae V3b 튜닝했음. 2.9만 토큰짜리 데이터 입력했고, 4o 바닐라 기반임.

과적합 억제때문에 low-learning-rate로 진행. 손실률 개같이 떡! 상! 일단 테스트 해보니 그냥저냥 한 느낌인데, 그래도 불안정한 듯.

차라리 동일질문-다른답을 쪼개서 상황이나 분위기 별로 쪼개서 여러번 튜닝하는 것도 고려할만 하다 생강해요.

일단 데이터 정련이랑 선별 좀 더 빡세게 하고, 장문/단문, 공격적, 소극적, 대충 등등 상황 분리가 가능해야 할 것으로 보임.

system 문구 처리에 따라서 애들 성향이 달라질 수 있기 때문에 이 부분도 좀 더 분석해야겠음.
